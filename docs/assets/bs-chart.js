...
async function _showChart(cell) {
...
async function _loadHistory(url) {
...
function _renderHistory(code, history) {
    ...
    const years = history.periods.map(function (period, index) {
...
function _renderYear(history, period, index) {
...
function _renderLane(title, roots, periodIndex, denominator, kind) {
    if (roots.length === 0) {
        return "";
    }
    return [
...
function _renderSegments(nodes, periodIndex, denominator, depth, path, laneKind) {
    if (depth > MAX_DEPTH) {
        return "";
    }
...
function _renderScopeLegend() {
    const swatches = SCOPE_DEPTH_LABELS.map(function (label, depth) {
...
function _groupRoots(roots) {
    const grouped = { assets: [], funding: [], other: [] };
...
function _unwrapStructuralRoot(root) {
    if (!_isStructuralWrapper(root)) {
        return [root];
    }
    return root.children.flatMap(_unwrapStructuralRoot);
}
function _isStructuralWrapper(root) {
    if (root.children.length === 0) {
        return false;
    }
    const hasOwnValue = root.values.some(function (value) {
...
function _displayLabel(sourceLabel, conceptName) {
...
function _splitConceptLabel(label) {
    return label
        .replace(/Abstract$/u, "")
        .replace(/LineItems$/u, "")
...
function _nodeValue(node, periodIndex) {
    return _toFiniteNumber(node.values[periodIndex]);
}
function _sumChildValues(node, periodIndex) {
